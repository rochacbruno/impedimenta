...
export BORG_PASSPHRASE='{{ borg_backup_users_passphrases[borg_backup_user.name] }}'
repository=usw-s001.rsync.net:borgbackup-{{ borg_backup_user.name }}
...
borg create \
    ...
    {% for directory in borg_backup_user.directories | default(('~/Documents', '~/Pictures')) %} {{ directory }} {% endfor %}
