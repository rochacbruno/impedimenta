...
set path=( /usr/bin /usr/local/bin /usr/ucb /bin /usr/ccs/bin /usr/bin/X11 /usr/local/ssl/bin /usr/local/php5/bin /usr/openwin/bin )
...
setenv MANPATH /usr/man:/usr/openwin/man:/usr/local/man:/usr/local/ssl/man
...
if ( $?prompt ) then
   ...
   if ( "`/bin/env | grep STY=`" == "" ) then 
        set prompt="`hostname`{`/usr/ucb/whoami`}\!: "
    else
        set screen=`echo $STY|awk -F. '{print $2}'`
        set prompt="`hostname` $screen {`/usr/ucb/whoami`}\!: "
    endif
