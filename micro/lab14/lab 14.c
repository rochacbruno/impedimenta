/*-============================================================================/
Jeremy Audet, for Micro, taught by Matt Gallagher. Lab 14.
...
The program structure is pretty straightforward. The body of the program uses an infinite while(1) loop. Every time this loop runs once, each of the four buttons on the board are checked for being pressed. If any one of the buttons is pressed, a message will be displayed saying so. A message will also be displayed that demonstrates the use of LCD_numint_out(int) and LCD_numachar_out(char). The program will wait after the message is displayed, until the user releases the button. If the user never releases the button they pushed, the while(1) loop will not be able to finish.
...
/* Your initial settings go here*/ 
    ...
    while(1) {
        // check button 1
        if(0 == (PTAD & 0x08)) {
            LCD_write_string("Button 1 (PTAD:3) pushed.\n");
            LCD_write_string("Printing number 3 in hex: ");
            LCD_numint_out(3);
        ...
        // check button 2
        if(0 == (PTAD & 0x04)) {
            LCD_write_string("Button 2 (PTAD:2) pushed.\n");
            LCD_write_string("Printing number 14 in hex: ");
            LCD_numint_out(14);
        ...
        // check button 3
        if(0 == (PTAD & 0x02)) {
            LCD_write_string("Button 3 (PTAD:1) pushed.\n");
            LCD_write_string("Printing hex ASCII value of '5': ");
            LCD_numchar_out('5');
        ...
        // check button 4
        if(0 == (PTAD & 0x20)) {
            LCD_write_string("Button 4 (PTAD:5) pushed.\n");
            LCD_write_string("Printing hex ASCII value of 'F': ");
            LCD_numchar_out('F');
