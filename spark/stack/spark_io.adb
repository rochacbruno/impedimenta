...
is
   ...
   Standard_Input_Constant  : constant File_Type := File_Type'(null, Stdin);
   Standard_Output_Constant : constant File_Type := File_Type'(null, Stdout);

   function Standard_Input return File_Type
   is
   begin
      return Standard_Input_Constant;
   end Standard_Input;

   function Standard_Output return File_Type
   is
   begin
      return Standard_Output_Constant;
   end Standard_Output;

   procedure Dispose is new Unchecked_Deallocation
     (Text_IO.File_Type, File_PTR);
   ...
   is
   begin
      File.File := new Text_IO.File_Type;
      Text_IO.Create (File.File.all,
                      Text_IO.Out_File,
   ...
   function File_Ref (File : File_Type) return Text_IO.File_Type;

   function File_Ref (File : File_Type) return Text_IO.File_Type
   is
   begin
      case File.IO_Sort is
         when Stdin     => return Text_IO.Standard_Input;
         when Stdout    => return Text_IO.Standard_Output;
   ...
   begin
      if Is_In (File) then
         Text_IO.Get_Immediate (File_Ref (File), Item);
         Status := Ok;
      else
         Item := Character'First;
         Status := Mode_Error;
   ...
   begin
      if Is_Out (File) then
         if Stop = 0 then
            ...
            Text_IO.Put (File_Ref (File), Item (Item'First .. Stop));
         else
            Pad := Stop - Item'Last;
            Text_IO.Put (File_Ref (File), Item);
            while Pad > 0 loop
               Text_IO.Put (File_Ref (File), ' ');
               Pad := Pad - 1;
            end loop;
         end if;
      end if;
